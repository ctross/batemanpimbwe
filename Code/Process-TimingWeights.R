...
p2 <- ggplot(plot1,aes(x=Age,y=Res1Mm)) +  ylab("Relative Value of Marriage to RS of Ego")+ xlab("Ego Age")+
      geom_ribbon(aes(ymin=Res1M1m,ymax=Res1M2m),alpha=0.5,fill="#e31a1c")+    xlim(c(0,65))+
      geom_line(aes(x=Age,y=Res1Mm),color="#e31a1c",size=2)+
...
p3 <- ggplot(plot1,aes(x=Age,y=Res1M1f)) +  ylab("Relative Value of Marriage to RS of Ego")+
      geom_ribbon(aes(ymin=Res1M1f,ymax=Res1M2f),alpha=0.5,fill="#253494")+  xlab("Ego Age")+   xlim(c(0,65))+
      geom_line(aes(x=Age,y=Res1Mf),color="#253494",size=2)+
      theme(axis.text=element_text(size=20), axis.title=element_text(size=22,face="bold"))
...
for(k in 1:Samples){
 ...
 for (a in 1:Age_males[i]){
  Ticker3 <- Ticker3 + (ifelse(PartnersByAge1_males[i,a+11]==0,0,1)+ifelse(PartnersByAge2_males[i,a+11]==0,0,1)+ifelse(PartnersByAge3_males[i,a+11]==0,0,1)+ifelse(PartnersByAge4_males[i,a+11]==0,0,1)+ifelse(PartnersByAge5_males[i,a+11]==0,0,1))*inv_logit(Delta_males[k,1] +  MarriageAgeEffect_males[k,a])
  }
...
for(k in 1:Samples){
 ...
 for (a in 1:Age_females[i]){
   Ticker3 <- Ticker3 + ifelse(PartnersByAge1_females[i,a+11]==0,0,1)*inv_logit(Delta_females[k,1]  + MarriageAgeEffect_females[k,a]);
   }
...
cv<-function(x){var(x)/(mean(x)^2)}
 ...
 Ims.f<-c()
      for(i in 1:Samples){
 Ims.m[i]<- cv(sample(SQ_SpouseYears_males[which(Age_males >(45-11)),i], replace = TRUE))
 Ims.f[i]<- cv(sample(SQ_SpouseYears_females[which(Age_females >(45-11)),i], replace = TRUE))
                              }

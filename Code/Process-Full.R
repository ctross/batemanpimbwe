...
##################################################################### Plot Densities
 ...
 aes(x=x,y=y), fill = '#e31a1c',alpha=0.4)
 ...
 aes(x=x,y=y), fill = '#253494',alpha=0.6)   + ylab("Density")+xlab("Elasticity: Spouse Number")+
 scale_fill_manual("Sex", values = c("#e31a1c","#253494"),guide=FALSE)+geom_vline(xintercept = 0,linetype=2) + theme_light() +
 theme(axis.text=element_text(size=22), axis.title=element_text(size=24,face="bold"))  + 
 annotate("text", x = -0.125, y = 1.255, label = "Males",colour = "white", size = 9) + 
 annotate("text", x = 0.093, y = 1.255, label = "Females",colour = "white", size = 9)
...
##################################################################### Plot Densities
 ...
 aes(x=x,y=y), fill = '#253494',alpha=0.6)   + ylab("Density")+xlab("Elasticity: Weighted Spouse Years")+
 scale_fill_manual("Sex", values = c("#e31a1c","#253494"),guide=FALSE)+geom_vline(xintercept = 1,linetype=2) + theme_light() +
 theme(axis.text=element_text(size=22), axis.title=element_text(size=24,face="bold")#,
 #legend.text=element_text(size=22), legend.title=element_text(size=24,face="bold")
 )+ 
 annotate("text", x = 0.98, y = 1.255, label = "Males",colour = "white", size = 9) + 
 annotate("text", x = 0.58, y = 1.255, label = "Females",colour = "white", size = 9) 
...
for(i in 1:Samples){
Res1[i,j]<-logistic(Delta[i,2] + SAE[i,j])
}
}
...
p6 <- ggplot(plot1,aes(x=Age,y=Median)) +  ylab("Probability Unmarried")+ xlab("Age")+ geom_line(aes(x=Age,y=Median, colour=Sex),size=1.25)+
    geom_ribbon(aes(ymin=Low,ymax=High,fill=Sex),alpha=0.5) + scale_fill_manual(values=c("#253494","#e31a1c")) +  scale_colour_manual(values=c("#253494","#e31a1c")) +
    theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))
...
cv<-function(x){var(x)/(mean(x)^2)}
 ...
 Ims.f<-c()
      for(i in 1:Samples){
 Ims.m[i]<- cv(sample(SQ_PQ_SpouseYears_males[which(Age_males >(45-11)),i], replace = TRUE))
 Ims.f[i]<- cv(sample(SQ_PQ_SpouseYears_females[which(Age_females >(45-11)),i], replace = TRUE))
                              }

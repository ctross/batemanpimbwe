...
for(i in 1:Samples){
Res1[i,j]<-logistic(Phi[i,1] + SAE_m[i,j])
}
}


for(i in 1:Samples){
Res1[i,]<-Res1[i,]/max(Res1[i,])
}
...
for( j in 1:MaxExposure){
for(i in 1:Samples){
Res1[i,j]<-logistic(Phi[i,1] + SAE_f[i,j])
}
}
...
p4 <- ggplot(plot1,aes(x=Age,y=Res1M1m)) +  ylab("Relative Value of Spouse to RS of Ego")+ xlab("Spouse Age")+ ylim(c(NA, 1))+
      geom_ribbon(aes(ymin=Res1M1m,ymax=Res1M2m),alpha=0.5,fill="#e31a1c")+
      geom_line(aes(x=Age,y=Res1Mm),color="#e31a1c",size=2)+        xlim(c(0,65))+
...
p5 <- ggplot(plot1,aes(x=Age,y=Res1M1f)) +  ylab("Relative Value of Spouse to RS of Ego")+
      geom_ribbon(aes(ymin=Res1M1f,ymax=Res1M2f),alpha=0.5,fill="#253494")+  xlab("Spouse Age")+  ylim(c(NA, 1))+
      geom_line(aes(x=Age,y=Res1Mf),color="#253494",size=2)+          xlim(c(0,65))+
      theme(axis.text=element_text(size=20), axis.title=element_text(size=22,face="bold"))
  ...
  for (a in 1:Age_males[i]){
   if(PartnersByAge1_males[i,a+11]==0){ESA[1] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,b]);}
          ESA[1] <- sum(Scrap);}
    else{ ESA[1] <-inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,PartnersByAge1_males[i,a+11]]);}}
   ...
   if(PartnersByAge2_males[i,a+11]==0){ESA[2] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,b]);}
          ESA[2] <- sum(Scrap);}
    else{ ESA[2] <-inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,PartnersByAge2_males[i,a+11]]);}}
   ...
   if(PartnersByAge3_males[i,a+11]==0){ESA[3] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,b]);}
          ESA[3] <- sum(Scrap);}
    else{ ESA[3] <-inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,PartnersByAge3_males[i,a+11]]);}}
   ...
   if(PartnersByAge4_males[i,a+11]==0){ESA[4] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,b]);}
          ESA[4] <- sum(Scrap);}
    else{ ESA[4] <-inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,PartnersByAge4_males[i,a+11]]);}}
   ...
   if(PartnersByAge5_males[i,a+11]==0){ESA[5] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,b]);}
          ESA[5] <- sum(Scrap);}
    else{ ESA[5] <-inv_logit(Phi_males[k,1] + SpouseAgeEffect_males[k,PartnersByAge5_males[i,a+11]]);}}
 ...
 for (i in 1:N_females){
  for (a in 1:Age_females[i]){
   if(PartnersByAge1_females[i,a+11]==0){EffectiveSpousesByAge_females[i,a,k] <-0;}
         ...
         for(b in 1:MaxExposure){
         Scrap[b]<-MissingDataWeights_females[b]*inv_logit(Phi_females[k,1] + SpouseAgeEffect_females[k,b]);}
         EffectiveSpousesByAge_females[i,a,k] <- sum(Scrap);}
     else{EffectiveSpousesByAge_females[i,a,k] <-inv_logit(Phi_females[k,1] + SpouseAgeEffect_females[k,PartnersByAge1_females[i,a+11]]);}}
     }}
   }
...
cv<-function(x){var(x)/(mean(x)^2)}
 ...
 Ims.f<-c()
      for(i in 1:Samples){
 Ims.m[i]<- cv(sample(PQ_SpouseYears_males[which(Age_males >(45-11)),i], replace = TRUE))
 Ims.f[i]<- cv(sample(PQ_SpouseYears_females[which(Age_females >(45-11)),i], replace = TRUE))
                              }
